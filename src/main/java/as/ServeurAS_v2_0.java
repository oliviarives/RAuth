...
import cmd.CmdServ;
import protocoles.tcp.GestionDiscussion;          // Checker TCP
import protocoles.tcp.GestionDiscussionManager;   // Manager TCP
import protocoles.udp.ServiceCheckerUDP;          // Checker UDP

import java.io.IOException;
import java.net.*;

public class ServeurAS_v2_0 {
    ...
    public static void main(String[] args) {
        ...
        try {
            // Checker : TCP + UDP (même port, protocoles différents)
            ...
            // Manager : TCP uniquement
            ServerSocket tcpManagerSocket = new ServerSocket(PORT_MANAGER);

            System.out.println("Serveur AS v2.0 démarré");
            ...
            Thread tCheckerTCP = new Thread(
                    () -> demarrerCheckerTCP(tcpCheckerSocket, listeAuth),
                    "ASv2-Checker-TCP"
            );

            Thread tCheckerUDP = new Thread(
                    () -> demarrerCheckerUDP(udpCheckerSocket, listeAuth),
                    "ASv2-Checker-UDP"
            );

            Thread tManagerTCP = new Thread(
                    () -> demarrerManagerTCP(tcpManagerSocket, listeAuth),
                    "ASv2-Manager-TCP"
            );
            ...
            System.out.println(CmdServ.ERROR.name() + " ServeurAS_v2_0 : " + e.getMessage());
        }
    }
    ...
    private static void demarrerCheckerTCP(ServerSocket server, ListeAuth listeAuth) {
        try (ServerSocket s = server) {
            System.out.println("[AS v2.0] Checker TCP prêt");
    ...
    private static void demarrerCheckerUDP(DatagramSocket socket, ListeAuth listeAuth) {
        try (DatagramSocket udp = socket) {
            System.out.println("[AS v2.0] Checker UDP prêt");
    ...
    private static void demarrerManagerTCP(ServerSocket server, ListeAuth listeAuth) {
        try (ServerSocket s = server) {
            System.out.println("[AS v2.0] Manager TCP prêt");
